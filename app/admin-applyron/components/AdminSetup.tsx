...
export default function AdminSetup({ onComplete }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (password.length < 6) {
      setError(messages.auth.setup.passwordTooShortClient);
      return;
    }
    if (password !== confirm) {
      setError(messages.auth.setup.passwordMismatch);
      return;
    }
    setLoading(true);
    try {
      ...
      if (!res.ok) {
        setError(data.error || messages.auth.setup.fallbackError);
      } else {
        onComplete();
      }
    } catch {
      setError(messages.auth.setup.networkError);
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-white">
            {messages.auth.setup.title}
          </h1>
          <p className="text-gray-400 mt-2">
            {messages.auth.setup.description}
          </p>
        </div>
        ...
        >
          <div className="mb-5">
            <label className="block text-sm font-medium text-gray-300 mb-2">
              {messages.auth.setup.passwordLabel}
            </label>
            <input
              type="password"
              value={password}
              onChange={(e) => setPassword(e.target.value)}
              placeholder={messages.auth.setup.passwordPlaceholder}
          ...
          <div className="mb-6">
            <label className="block text-sm font-medium text-gray-300 mb-2">
              {messages.auth.setup.confirmLabel}
            </label>
            <input
              type="password"
              value={confirm}
              onChange={(e) => setConfirm(e.target.value)}
              placeholder={messages.auth.setup.confirmPlaceholder}
              autoComplete="new-password"
              className="w-full bg-[#0d0030] border border-purple-500/30 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 transition"
            />
          </div>
          ...
          >
            {loading
              ? messages.auth.setup.submitting
              : messages.auth.setup.submit}
          </button>
        </form>
      </div>
    </div>
